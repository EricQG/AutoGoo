...
2. **执行后归档**：把本次任务的目标、计划、步骤证据、产物、验证结果、指标、问题处理和可复用经验写回 Goo-wiki。
...
**路径检测**：按 `AUTO_GOO_WIKI_DIR` → `.goo/config.json.wiki_dir` → `~/.auto-goo/config.json.wiki_dir` → `~/workspace/Goo-wiki` 解析 wiki 目录，并检查 `<wiki_dir>/CLAUDE.md` 是否存在（详见 `setup.md`）。不存在则降级为 fallback。
...
12. 任务完成后向 Goo-wiki/log.md 追加一条活动日志
13. 不要写入 raw/ 目录
14. 输出目录优先级：Goo-wiki/wiki/ > .goo/obsidian/（fallback）
15. 子步骤内容内联在主任务笔记中，用 --- 分隔，不拆为独立文件
```
