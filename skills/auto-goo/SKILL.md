...
9. 写入目标 config，默认结构参考 `skills/auto-goo/templates/config.example.json`；项目级配置必须记录 `archive.project_slug`、`archive.project_dir` 和 `archive.fallback_project_dir`。
10. 如果是 `--project` 且 Goo-wiki 可用，必须询问用户是否在项目 `CLAUDE.md` 中加入或更新 AutoGoo marker 包裹的项目归档原则和要求；只改该段，不覆盖用户已有项目指引。非交互场景默认不写，需传 `--update-claude-md` 明确写入；用户传 `--skip-claude-md` 时跳过。
11. 展示推荐 SessionStart hooks，但不要自动覆盖 `.claude/settings.json`，除非用户明确要求。
...
- 如果执行前发现当前对话里有新决策尚未写入 plan，先归档旧 plan，再更新当前 plan 或写入 Goo-wiki 项目路径 `context/`，然后再执行；Goo-wiki 不可用时写 `.goo/obsidian/<project-slug>/context/`。
...
**Subagent 显式分工**：每个 step 必须包含 `subagent` 字段。允许值为 `research`、`implementer`、`optimizer`、`evaluator`、`reviewer`、`recorder`。`type` 表示步骤类型，`subagent` 表示执行角色；调度时优先按 `subagent` 选择 prompt 模板。
